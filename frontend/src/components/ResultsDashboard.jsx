    ...
    >
      ...
      <div id="risk-section" className="scroll-mt-24 space-y-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StrategyCard title="Conservative" rate="8.5%" emi={results.risk.emi_monthly * 0.9} stress="Lowest Risk" desc="Prioritizes long-term safety and minimal monthly burden." />
          <StrategyCard title="Balanced" rate="9.2%" emi={results.risk.emi_monthly} stress="Recommended" desc="Optimal balance between interest cost and liquidity." highlighted />
          <StrategyCard title="Aggressive" rate="10.5%" emi={results.risk.emi_monthly * 1.3} stress="High Savings" desc="Aggressive repayment to minimize total interest paid." />
        </div>
      ...
      <div id="stress-test-section" className={`scroll-mt-24 transition-all duration-500 rounded-[2rem] p-8 border ${showStressTest ? 'bg-rose-950/10 border-rose-500/50 shadow-[0_0_50px_rgba(244,63,94,0.2)]' : 'bg-[#111827]/50 border-zinc-800/60'}`}>
      ...
      <div className="space-y-12">
        {/* Opportunity Cost - Vertical Stack */}
        <div id="opportunity-section" className="scroll-mt-24 bg-[#111827] border border-zinc-800/60 rounded-[3rem] p-10">
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex bg-[#0b0f19] rounded-xl p-1 border border-zinc-800">
              <button onClick={() => setOppView('line')} className={`px-6 py-2.5 rounded-lg text-xs font-black transition-all ${oppView === 'line' ? 'bg-emerald-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>LINE</button>
              <button onClick={() => setOppView('area')} className={`px-6 py-2.5 rounded-lg text-xs font-black transition-all ${oppView === 'area' ? 'bg-emerald-600 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>AREA</button>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              {oppView === 'line' ? (
                <LineChart data={results.opportunity?.chart_data}>
                  ...
                  <XAxis dataKey="year" fontSize={12} tickMargin={15} stroke="#4b5563" fontWeight={800} />
                  <YAxis tickFormatter={rupeeFormatter} fontSize={12} stroke="#4b5563" fontWeight={800} />
                  <Tooltip content={<CustomTooltip />} />
                  <Line type="monotone" dataKey="cumulative_emi_paid" stroke="#ef4444" strokeWidth={5} dot={false} name="EMI Paid" />
                  <Line type="monotone" dataKey="investment_value" stroke="#10b981" strokeWidth={5} dot={false} name="SIP Value" />
                </LineChart>
              ) : (
                <AreaChart data={results.opportunity?.chart_data}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" vertical={false} />
                  <XAxis dataKey="year" fontSize={12} stroke="#4b5563" fontWeight={800} />
                  <YAxis tickFormatter={rupeeFormatter} fontSize={12} stroke="#4b5563" fontWeight={800} />
                  <Tooltip content={<CustomTooltip />} />
                  <Area type="monotone" dataKey="investment_value" stroke="#10b981" strokeWidth={4} fillOpacity={1} fill="url(#colorSIP)" name="SIP Value" />
                </AreaChart>
              )}
            </ResponsiveContainer>
          </div>
        ...
        <div id="rent-section" className="scroll-mt-24 bg-[#111827] border border-zinc-800/60 rounded-[3rem] p-10">
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex gap-4">
              {[10, 20, 30].map(y => (
                <button key={y} onClick={() => setShowYears(y)} className={`text-xs font-black px-6 py-2.5 rounded-xl border transition-all ${showYears === y ? 'border-emerald-500 text-emerald-500 bg-emerald-500/10' : 'border-zinc-800 text-zinc-500'}`}>
                  {y}Y HORIZON
                </button>
              ))}
            </div>
          ...
          <div className="h-[500px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={filteredDebtData}>
                ...
                <YAxis tickFormatter={rupeeFormatter} fontSize={12} stroke="#4b5563" fontWeight={800} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="debt_cumulative" fill="#ef4444" name="Loan Cost" radius={[6, 6, 0, 0]} barSize={20} />
                <Bar dataKey="rent_cumulative" fill="#3b82f6" name="Rent Cost" radius={[6, 6, 0, 0]} barSize={20} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-8">
            <div className="p-8 bg-zinc-950/50 rounded-[2rem] border border-zinc-800">
              ...
              <p className="text-5xl font-black text-white">{results.optimize?.recommended_tenure_years || '--'} <span className="text-2xl text-zinc-600">Years</span></p>
            </div>
            <div className="p-8 bg-emerald-500/5 rounded-[2rem] border border-emerald-500/20">
              ...
              <p className="text-5xl font-black text-emerald-400 tracking-tight">₹{results.optimize?.interest_saved?.toLocaleString('en-IN') || '0'}</p>
            </div>
          </div>
          <div className="flex items-center">
            <p className="text-xl text-zinc-400 leading-relaxed italic border-l-4 border-emerald-500 pl-10 font-medium">
...
function StrategyCard({ title, rate, emi, stress, desc, highlighted }) {
  return (
    <motion.div 
      whileHover={{ y: -10, borderColor: highlighted ? '#10b981' : '#52525b' }}
      className={`p-10 rounded-[2.5rem] border transition-all flex flex-col justify-between ${highlighted ? 'bg-[#1a2333] border-emerald-500/40 shadow-2xl shadow-emerald-950/40' : 'bg-[#111827] border-zinc-800/60'}`}
    >
      <div>
        <div className="flex justify-between items-center mb-6">
          <h5 className="text-base font-black text-zinc-100 uppercase tracking-[0.2em]">{title}</h5>
          <span className={`text-xs px-3 py-1.5 rounded-xl font-black uppercase tracking-tighter ${highlighted ? 'bg-emerald-500 text-black' : 'bg-zinc-800 text-zinc-500'}`}>
            {stress}
          </span>
        </div>
        <p className="text-base text-zinc-500 mb-10 leading-relaxed font-medium">{desc}</p>
        <div className="space-y-4 border-t border-zinc-800/80 pt-8">
          <div className="flex justify-between text-base">
            <span className="text-zinc-500 font-black uppercase tracking-widest text-xs">ROI</span> 
            <span className="text-zinc-100 font-bold">{rate}</span>
          </div>
          <div className="flex justify-between items-baseline">
            <span className="text-zinc-500 font-black uppercase tracking-widest text-xs">Monthly EMI</span> 
            <span className="text-3xl text-emerald-400 font-black tracking-tight">₹{Math.round(emi).toLocaleString('en-IN')}</span>
          </div>
        </div>
      </div>
    </motion.div>
